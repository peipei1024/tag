...
import com.leancloud.im.tag.event.Chat_MemberLetterEvent;
 ...
 * 联系人列表，快速滑动字母导航 View
 * 此处仅在滑动或点击时发送 Chat_MemberLetterEvent，接收放自己处理相关逻辑
 * 注意：因为长按事件等触发，有可能重复发送
 */
public class Chat_LetterView extends LinearLayout {

  public Chat_LetterView(Context context) {
    super(context);
    setOrientation(VERTICAL);
    updateLetters();
  }

  public Chat_LetterView(Context context, AttributeSet attrs) {
    super(context, attrs);
    setOrientation(VERTICAL);
    updateLetters();
  }
      ...
      public boolean onTouch(View v, MotionEvent event) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
          ...
          if (y > child.getTop() && y < child.getBottom()) {
            Chat_MemberLetterEvent letterEvent = new Chat_MemberLetterEvent();
